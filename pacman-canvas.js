/*Pacman by Doug Bruce
_______            _____              _____           __        __          ______         __     ___
|    __   \         /   __    \         /  ___    \       |    \     /   |        /   __    \       |    \  |    |
|    |  \   \      /    /   \    \      /   /     \__|      |     \   /    |      /    /    \   \      |     \ |    |
|    |  /   /     |    |___|   |     |   |                  |     \ /      |     |    |___|   |      |      \|    |
|    ___/       |     ___     |     |   |       __       |               |     |    ____    |     |    |        | 
...
/* ----- Global Variables ---------------------------------------- */
	...
	function buildWall(context,gridX,gridY,width,height) {
		width = width*2-1;
		height = height*2-1;
		context.fillRect(pacman.radius/3+gridX*2*pacman.radius,pacman.radius/3+gridY*2*pacman.radius, width*pacman.radius, height*pacman.radius);
	}
	...
	function Game() {
		...
		this.init = function (state) {
			...
			if (pinky == null) {
				pinky = new Ghost(24*pacman.radius,20*pacman.radius,'img/pinky.svg');
				inky = new Ghost(25*pacman.radius,20*pacman.radius,'img/inky.svg');
				blinky = new Ghost(26*pacman.radius,20*pacman.radius,'img/blinky.svg');
				clyde = new Ghost(27*pacman.radius,20*pacman.radius,'img/clyde.svg');
			}
			else {
				//console.log("ghosts reset");
				pinky.setPosition(24*pacman.radius,20*pacman.radius);
				pinky.undazzle();
				inky.setPosition(25*pacman.radius,20*pacman.radius);
				inky.undazzle();
				blinky.setPosition(26*pacman.radius,20*pacman.radius);
				blinky.undazzle();
				clyde.setPosition(27*pacman.radius,20*pacman.radius);
				clyde.undazzle();
			}
	...
	function Ghost(posX, posY, image) {
		...
		this.reset = function() {
			this.posX = 24*pacman.radius;
			this.posY = 20*pacman.radius;
			this.undazzle();
		}
		...
		this.checkCollision = function() {
			...
			if ((this.dirX == 1) && (gridAheadX < 28)) gridAheadX += 1;
			if ((this.dirY == 1) && (gridAheadY < 22)) gridAheadY += 1;//console.log(this.dirY);
	...
	function Figure() {
		...
		this.getGridPosX = function() {
			return (this.posX - (this.posX % 20))/20;
		}
		this.getGridPosY = function() {
			return (this.posY - (this.posY % 20))/20;
		}
	...
	function pacman() {
		this.radius = 10.00;//15
		this.posX = 0;
		this.posY = 9*2*this.radius;//6*2*this.radius
		this.angle1 = 0.25;
		this.angle2 = 1.75;
		...
		this.checkCollisions = function () {
			...
			if ((this.stuckX == 0) && (this.stuckY == 0) && this.frozen == false) {
				...
				// get the field 1 ahead to check wall collisions
				if ((this.dirX == 1) && (gridAheadX < 28)) gridAheadX += 1;
				if ((this.dirY == 1) && (gridAheadY < 22)) gridAheadY += 1;
				var fieldAhead = game.map.posY[gridAheadY].posX[gridAheadX];
				...
				if ((fieldAhead.type === "wall") || (fieldAhead.type === "door")) {
					this.stuckX = this.dirX; //console.log('before x is '+this.posX);
					this.stuckY = this.dirY; //console.log('before y is '+this.posY);
					pacman.stop();
					// get out of the wall
					if ((this.stuckX == 1) && ((this.posX % 20) != 0)) this.posX -= this.posX % 20; //console.log('after x is '+this.posX);
					if ((this.stuckY == 1) && ((this.posY % 20) != 0)) this.posY -= this.posY % 20;//console.log('after y is '+this.posY);
...
function checkAppCache() {
	...
	$(document).ready(function() {
		...
		function renderContent()
		{
			...
			//horizontal outer
			buildWall(context,0,0,28,1);
			buildWall(context,0,21,28,1);
			
			// vertical outer
			buildWall(context,0,0,1,8);
			buildWall(context,0,10,1,12);
			buildWall(context,27,0,1,9);
			buildWall(context,27,10,1,12);
			
			// ghost base
			buildWall(context,10,9,1,3);
			buildWall(context,17,9,1,3);//r
			buildWall(context,10,11,8,2);
			
			// ghost base door
			context.fillRect(13*2*pacman.radius,pacman.radius/2+9*2*pacman.radius+5, 4*pacman.radius, 1);
			
			// single blocks
			buildWall(context,13,0,2,4);
			buildWall(context,13,5,2,3);
			buildWall(context,10,5,8,1);
			buildWall(context,10,12,8,1);
			buildWall(context,13,12,2,4);
			buildWall(context,2,2,4,2);
			buildWall(context,22,2,4,2);//r
			buildWall(context,7,2,5,2);
			buildWall(context,16,2,5,2);//r
			buildWall(context,7,5,2,4);
			buildWall(context,19,5,2,4);//r
			buildWall(context,7,7,5,1);
			buildWall(context,16,7,5,1);//r
			buildWall(context,0,7,6,2);
			buildWall(context,22,7,6,2);//r
			buildWall(context,0,10,6,3);
			buildWall(context,22,10,6,3);//r
			buildWall(context,2,5,4,1);
			buildWall(context,22,5,4,1);//r
			buildWall(context,7,10,2,3);
			buildWall(context,19,10,2,3);//r
			buildWall(context,7,14,5,2);
			buildWall(context,16,14,5,2);//r
			buildWall(context,2,14,4,1);
			buildWall(context,22,14,4,1);//r
			buildWall(context,4,14,2,4);
			buildWall(context,22,14,2,4);//r
			buildWall(context,7,17,2,3);
			buildWall(context,19,17,2,3);//r
			buildWall(context,13,17,2,3);
			buildWall(context,10,17,8,1);
			buildWall(context,2,19,10,1);
			buildWall(context,16,19,10,1);//r
			buildWall(context,0,16,3,2);
			buildWall(context,25,16,3,2);//r
			buildWall(context,10,9,3,1);
			buildWall(context,15,9,3,1);
